...
% COMS 4721
% Homework 3
% Part 3 - Binary Logistic Classifier
...
%% Call to Logistic Classifer

omegai = logistic_classifier(Bt, Blabel);
...
for i = 1 : size(Xtrain,2)
    ...
    ftxi = sign((entry_train' * omegai)); % predicted answer
...
for k = 1 : size(Xtrain,2)
    ...
    ftw = sign(entryw' * omegai); % predicted answer
...
for i = 1 : size(Xtrain,2)
    entry = Xtrain(:,i);
    ft = sign(entry'*omegai); % predicted answer
...
%% Logistic Classifier on Training and Testing Set Without Boosting
% 82% correct

w_log = logistic_classifier(Xtrain,labeltrain);

log_right = 0;
for i = 1 : size(Xtest,2)
    entry = Xtest(:,i);
    
    ftxi = sign((entry'*w_log)); % predicted answer
...
log_accuracy = log_right / size(Xtest,2);
...
title('Accuracy of Training and Testing Predictions - Logistic Classifier');
...
title('Epsilon and Alpha over T iterations - Logistic Classifier');
xlabel('T iterations');
